    ...
    <div style={{ display: 'flex', flexDirection: 'column', height: '100%', gap: 20 }}>
      ...
      <div className="reception-grid-layout">
         ...
         <div style={{ display: 'flex', flexDirection: 'column', gap: 20, overflowY: 'auto', paddingRight: 5 }}>
            ...
            {!selectedMember ? (
               <div className="glass-card" style={{ flex: 1, padding: 30, display: 'flex', flexDirection: 'column', alignItems: 'center', justifyContent: 'center', border: '1px solid var(--green-10)', borderRadius: 24, minHeight: 250 }}>
                  <Activity size={60} style={{ color: 'var(--neon-green)', opacity: 0.1, marginBottom: 20 }} />
                  <div style={{ position: 'relative', width: '100%' }}>
                    <Search size={18} style={{ position: 'absolute', left: 15, top: 15, color: 'var(--text-muted)' }} />
                    <input 
               ...
               <div className="glass-card" style={{ flex: 1, padding: 20, border: `1px solid ${selectedMember.debt > 0 ? 'var(--danger-red)' : 'var(--green-20)'}`, borderRadius: 24, display:'flex', flexDirection:'column' }}>
                    ...
                    <div style={{ display: 'grid', gridTemplateColumns: 'repeat(3, 1fr)', gap: 8, marginBottom: 15, maxHeight: 180, overflowY: 'auto', padding: '5px' }}>
                      {products.slice(0, 12).map(p => (
                        <div key={p.id} onClick={() => addToCart(p)} className="quick-product-card" style={{ position: 'relative', background: 'rgba(255,255,255,0.02)', border: '1px solid rgba(255,255,255,0.05)', borderRadius: 12, padding: '10px 5px', textAlign: 'center', cursor: 'pointer', transition: '0.3s' }}>
                          ...
                          {p.stock <= p.minStock && (
                            <div style={{ position: 'absolute', top: -5, right: -5, width: 12, height: 12, borderRadius: '50%', background: 'var(--danger-red)', border: '2px solid #000' }} />
                          )}
                        </div>
                      ))}
                    </div>
                  ...
                  <div style={{ flex:1, overflowY:'auto', background:'rgba(0,0,0,0.2)', borderRadius:12, padding:10, marginBottom:15, minHeight: 80 }}>
                     {cart.length === 0 ? (
                        <div style={{ textAlign:'center', color:'var(--text-muted)', fontSize:10, paddingTop: 30 }}>CARRITO VACÍO</div>
                     ) : cart.map(item => (
                        <div key={item.id} style={{ display:'flex', justifyContent:'space-between', fontSize:11, marginBottom:5 }}>
                           <span>{item.qty}x {item.name}</span>
                           <button onClick={() => removeFromCart(item.id)} style={{ color:'var(--danger-red)', background:'none', border:'none', cursor:'pointer' }}><X size={12}/></button>
                        </div>
                     ))}
                  </div>
                     ...
                     <div style={{ display: 'flex', flexDirection: 'column', gap: 20 }}>
                        <div className="glass-card" style={{ padding: 25, borderRadius: 24, border: '1px solid var(--green-20)', background: 'rgba(0,255,136,0.03)' }}>
                           ...
                           <div style={{ display: 'grid', gridTemplateColumns: '1fr 1fr 1fr', gap: 12, marginBottom: 25 }}>
                              {[
                                 { id: 'srv_mes', label: 'MENSUAL', price: 75000, color: 'var(--neon-green)', icon: <TrendingUp size={14}/> },
                                 { id: 'srv_sem', label: 'SEMANAL', price: 25000, color: '#00E5FF', icon: <Calendar size={14}/> },
                                 { id: 'srv_dia', label: 'PAGO DÍA', price: 5000, color: '#FFD600', icon: <Zap size={14}/> }
                              ].map(srv => (
                                 <button key={srv.id} onClick={() => addToCart({ ...srv, name: srv.label, sellPrice: srv.price, category: 'Servicio' })} style={{ background: 'rgba(255,255,255,0.02)', border: `1px solid ${srv.color}20`, borderRadius: 16, padding: '18px 10px', color: '#fff', cursor: 'pointer', transition: '0.3s', display:'flex', flexDirection:'column', alignItems:'center', gap: 8 }}>
                                    <div style={{ color: srv.color }}>{srv.icon}</div>
                                    <div style={{ fontSize: 9, fontWeight: 950, letterSpacing: 1 }}>{srv.label}</div>
                                    <div style={{ fontSize: 14, fontWeight: 950 }}>${(srv.price/1000).toFixed(0)}K</div>
                                 </button>
                              ))}
                           </div>
